...
<script>
	//閲覧不可ページ
	let lockArray = [
		<?php foreach ($pdo->query('select * from locklist') as $row) : ?> '<?= $row['page'] ?>',
		<?php endforeach; ?>
	];
	let unlock = <?= $countIp ?>;
</script>
...
<nav id="sidebar" class="">
	<div id="header-wrapper">
		<form action="../../common/search/index.php" method="get" id="search">
			<select id="search-select" name="cate">
				<option value="all" selected="">検索対象すべて</option>
				<option value="js">JS</option>
				<option value="php">PHP</option>
				<option value="ps">photoshop参考書</option>
				<option value="slimBook">slim参考書</option>
				<option value="css">CSS</option>
				<option value="html">HTML</option>
			</select>
			<div class="keyword">
				<input type="text" id="word" name="word" placeholder="全ページ検索">

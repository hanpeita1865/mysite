...
function menuSort() {
	...
	$('.side-link li').each(function (e, elm) {
		...
		if (!$(this).hasClass('dummy')) {
			...
			if($(this).children('.sub-menu').length && $(this).parent('ul').hasClass('side-link')){
				$(this).children('a').after('<i class="fa fa-chevron-circle-down" aria-hidden="true"></i>');
...
//サブメニュー開閉
$('.side-link li .fa-chevron-circle-down').on('click', function () {
...
function sortConfirm(){
	...
	menuList = menuList.replace(/<li class="dummy"><\/li>/g, '');
...
$('.folder-rename').on('click', function (e, obj) {
	...
	if (folderReName == null || folderName == folderReName) {//キャンセルまたは名前を変えずにokを押した場合
		...
		$.ajax({
			...
			$.ajax({
				url: 'lockdata_edit.php',
				type: 'POST',
				cache: false,
				data: { post_folder: folderName, post_reName: folderReName }
			}).done(function (data) {
				//通信成功時の処理
				//alert('DBの閲覧不可の登録名を修正しました。');
...
$('.btn-sub-close').on('click',function(){
	$('.side-link > li').toggleClass('sub-open');
});
...
function objDrop(obj) {
	...
	obj.on('drop', function (e) {
		...
		if (obj.hasClass('emp-ul')) {//ボックスが空のとき実行
			...
			$(this).before('<i class="fa fa-chevron-circle-down" aria-hidden="true"></i>');
